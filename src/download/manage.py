...
def download(
    ...
    if selected_website == "lelscans":
        status = lelscans.init_download(
            selected_website, chapter_file_path, selected_manga_name, chapter_name
        )

    elif selected_website == "fmteam":
        status = fmteam.init_download(
            selected_website, chapter_file_path, selected_manga_name, chapter_name, DRIVER
        )

    elif selected_website == "tcbscans":
        status = tcbscans.init_download(
            selected_website, chapter_file_path, selected_manga_name, chapter_name
        )

    elif selected_website == "lelmanga":
        status = lelmanga.init_download(
            selected_website, chapter_file_path, selected_manga_name, chapter_name
        )

    elif selected_website == "manganelo":
        status = manganelo.init_download(
            selected_website, chapter_file_path, selected_manga_name, chapter_name
        )

    elif selected_website == "mangamoins":
        status = mangamoins.init_download(
            selected_website, chapter_file_path, selected_manga_name, chapter_name
        )

    elif selected_website == "mangasaki":
        status = mangasaki.init_download(
            selected_website, chapter_file_path, selected_manga_name, chapter_name, DRIVER
        )

    elif selected_website == "lhtranslation":
        status = lhtranslation.init_download(
            selected_website, chapter_file_path, selected_manga_name, chapter_name
        )
